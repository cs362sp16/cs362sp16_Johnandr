...
//	Get 10 random cards for the domion game
void getKingdomCards(int *hand) {
	srand(time(NULL));
	int selected = 0;
	while(selected < 10) {
		int card = (int) (Random() * (treasure_map - adventurer) + adventurer);
...
//	Took from domion.c code for ease
...
int findSupplyCard(int *hand, int budget) 
{
	int found = 0;
	while(!found) 
	{
		int pos = (int) (Random() * 10);
		if(myGetCost(hand[pos]) <= budget) 
			return hand[pos];
	}
	return -1;
}
		...
		while(p->numActions > 0 && tried < 1000) 
		{
				...
				(card != estate) ) 
			{
				...
				case council_room:
						ret = playCard(pos,-1,-1,-1,p);
						if(ret == -1) 
							printf("Failed to play council_room card\n");
						else 
				...
				case feast:
						...
						if(ret == -1) 
							printf("Failed to play feast card\n");
						else 
				...
				case gardens:
						ret = playCard(pos,-1,-1,-1,p);
						if(ret == -1) 
							printf("Failed to play gardens card\n");
                        else 
				...
				case mine:
						mon = getMoneyType();
						//printf("\t\t\t%d\n",myGetCost(mon));
                        ret = playCard(pos,findHandMoney(p),mon,-1,p);
                        if(ret == -1) 
							printf("Failed to play mine card %d\n",mon);
                        else 
				...
				case remodel:
                        ...
                        if(ret == -1) 
							printf("Failed to play remodel card\n");
                        else 
                ...
                case baron:
						...
						if(ret == -1) 
							printf("Failed to play baron card\n");
						else 
				...
				case great_hall:
						ret = playCard(pos,-1,-1,-1,p);
       	                if(ret == -1) 
							printf("Failed to play great_hall card\n");
          	            else 
                ...
                case minion:
                        ...
                        if(ret == -1) 
							printf("Failed to play minion card\n");
                        else 
